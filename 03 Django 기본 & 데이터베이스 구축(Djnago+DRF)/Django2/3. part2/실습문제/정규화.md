...
| 컬럼명        | 타입           | 키                        | 설명       |
| ---------- | ------------ | ------------------------ | -------- |
| student_id | INT          | PK                       | 학생 고유 ID |
| name       | VARCHAR(50)  |                          | 학생 이름    |
| birth_date | DATE         |                          | 생년월일     |
| phone      | VARCHAR(20)  |                          | 연락처      |
| email      | VARCHAR(100) |                          | 이메일      |
| address    | VARCHAR(255) |                          | 주소       |
| dept_id    | INT          | FK → Department(dept_id) | 소속 학과    |
...
| 컬럼명         | 타입          | 키                        | 설명           |
| ----------- | ----------- | ------------------------ | ------------ |
| guardian_id | INT         | PK                       | 보호자 ID       |
| student_id  | INT         | FK → Student(student_id) | 어떤 학생의 보호자인지 |
| name        | VARCHAR(50) |                          | 보호자 이름       |
| relation    | VARCHAR(30) |                          | 관계(부/모/기타)   |
| phone       | VARCHAR(20) |                          | 연락처          |

> “학생 기본 정보”에 보호자/비상연락망이 포함되는 경우가 많아 보통 1:N으로 분리합니다.

---

### 2) 관계 요약표 (Cardinality)

|관계(부모 → 자식)|관계 종류|의미|
|---|---|---|
|Department → Student|1:N|한 학과에 여러 학생이 소속될 수 있음|
|Student → Guardian|1:N|한 학생은 여러 비상연락망(보호자)을 가질 수 있음|
...
```
[Department] 1 -------- N [Student] 1 -------- N [Guardian]
   PK dept_id              PK student_id          PK guardian_id
...
```
UNIQUE(권장):
- Student(email)  (이메일을 계정처럼 쓰면 유니크)
- Student(phone)  (정책에 따라 유니크 가능)
- Department(dept_name) (학과명이 중복되지 않게)
```

---
![[Group 176.png]]

![[Pasted image 20260111183521.png]]