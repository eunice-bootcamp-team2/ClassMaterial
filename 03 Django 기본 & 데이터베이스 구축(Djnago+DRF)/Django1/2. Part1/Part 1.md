...
◽ URL에 이름 부여 (`name`)
	URL 경로에 이름을 부여하여, 나중에 템플릿(html)이나 코드에서 하드코딩 없이링크 참조가 가능합니다.
...
💬 비즈니스 로직이란?
	...
	즉, 이 서비스에서 ‘이럴 때는 이렇게 해야 한다’고 정해둔 실제 일 처리 규칙입니다.

요청부터 화면 반환까지 전체 워크플로우 중 하나의 기능이 동작할 때 실제로는 3가지 레이어로 구분합니다.
```
[ 요청 ] → [ 판단/계산 ]:여기에 해당됨 → [ 응답 ]
```

###### 이중 비즈니스 로직은 아래 표에 해당합니다.
| 단계         | 역할     | 예                |
| ---------- | ------ | ---------------- |
| 요청 처리      | 기술적 처리 | HTTP 요청, JSON 파싱 |
| 🔴 비즈니스 로직 | 도메인 규칙 | 계산, 조건, 제한       |
| 응답 반환      | 표현/출력  | HTML, JSON 응답    |
