...
import requests                # 웹페이지에 HTTP 요청을 보내서 HTML을 가져오기 위한 라이브러리
from bs4 import BeautifulSoup  # HTML 문서를 분석(파싱)해서 원하는 태그 데이터를 추출하기 위한 라이브러리
from urllib.parse import urljoin  # 상대 경로 URL을 절대 경로 URL로 변환할 때 사용하는 유틸리티
