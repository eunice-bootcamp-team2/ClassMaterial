...
```python
exec("""
from apps.crawling.models import CrawlTarget, CrawlRawData
import random
import hashlib

target = CrawlTarget.objects.first()
print("target =", target)

adjectives = ["좋다", "별로다", "괜찮다", "최고다", "아쉽다"]
features = ["보습력", "흡수력", "향", "가격", "사용감"]
feelings = ["만족", "불만족", "무난", "추천", "비추천"]

def make_review():
    return f"{random.choice(features)}이 {random.choice(adjectives)}. 전체적으로 {random.choice(feelings)}합니다."

reviews = []

for i in range(100):
    review_text = make_review()
    unique_key = hashlib.sha256(
        f"{target.site}:review:{target.id}:{i}:{review_text}".encode("utf-8")
    ).hexdigest()
    ...
    reviews.append(
        CrawlRawData(
            ...
            page_title=target.title or "",
            item_title=target.title or "",
            item_url=target.url,
...
result = CrawlRawData.objects.bulk_create(reviews, ignore_conflicts=True)
print("저장 완료 개수 =", len(result))
print("현재 review 개수 =", CrawlRawData.objects.filter(target=target, record_type="review").count())
""")
```
...
user = User.objects.get(username="youjung7")

positive_reviews = [
    "보습력이 좋아서 겨울에 쓰기 좋아요.",
    "수분감이 오래가서 건조할 때 만족스러워요.",
    "촉촉하고 발림성도 좋아서 자주 손이 가요.",
    "보습감이 괜찮고 피부가 당기지 않아요.",
    "가격도 괜찮고 사용감도 무난해서 만족합니다.",
]

negative_reviews = [
    "보습력이 약해서 금방 건조해져요.",
    "수분감이 부족해서 기대보다 별로였어요.",
    "발림성은 괜찮지만 보습 지속력이 아쉬워요.",
    "사용하고 나서도 피부가 금방 당기는 느낌이에요.",
    "가격 대비 효과가 크지 않아서 실망했어요.",
]

neutral_reviews = [
    "무난하게 사용할 수 있는 제품이에요.",
    "장점도 있고 아쉬운 점도 있는 편이에요.",
    "보습감은 평범하고 발림성도 보통이에요.",
    "특별히 나쁘진 않지만 아주 좋지도 않았어요.",
    "전체적으로 무난한 사용감입니다.",
]
...
Review.objects.bulk_create(samples)
print("추가 저장 개수 =", len(samples))
print("상품 5번 공개 리뷰 수 =", Review.objects.filter(product_id=5, is_public=True).count())
""")
```