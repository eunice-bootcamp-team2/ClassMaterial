...
모델 구조가 바뀌었기 때문 DB를 초기화(리셋) 하는 작업입니다.
Django는 `makemigrations`, `migrate`가 있지만 FastAPI + SQLAlchemy 구조에서는 자동으로 DB 구조를 바꿔주지 않습니다.
...
그래서 가장 쉬운 방법인 삭제후 재생성하면 내부에서 최신 모델 기준으로 DB 다시 생성됩니다.
```python
Base.metadata.create_all(bind=engine)
```
