이 예제는 회원(User)을 등록하고, 각 사용자가 자신의 물건(Item)을 등록하거나 조회할 수 있는 간단한 게시판 형태의 API를 만들면서 데이터베이스를 연결하는 방법을 설명합니다.

SQLAlchemy는 Python에서 데이터베이스를 다루기 위해 사용하는 ORM(Object Relational Mapper) 라이브러리입니다.  
ORM을 사용하면 SQL 문을 직접 작성하지 않아도 Python 코드로 데이터베이스를 제어할 수 있습니다.

예를 들어 Django에서는 `models.Model`을 사용하여 Python 클래스로 데이터베이스 테이블을 정의합니다.  

SQLAlchemy도 이와 비슷하게 Python 클래스를 작성하면 그것이 데이터베이스 테이블 구조로 연결됩니다.

차이점은 다음과 같습니다.
- Django는 ORM이 프레임워크 안에 기본적으로 포함되어 있습니다.
- FastAPI는 ORM이 기본으로 제공되지 않기 때문에 SQLAlchemy와 같은 외부 라이브러리를 설치하여 사용해야 합니다.
    
그래서 FastAPI에서 데이터베이스를 사용하려면 다음과 같이 SQLAlchemy를 설치해 사용합니다.
```bash
uv pip install sqlalchemy
```
---

본격적으로 DB를 연결하여 간단하게 생성하기 조회하기를 만들려고 하는데 Django는 엄격한 규칙이 존재하지만 FastAPI는 규칙이 없습니다. 즉 main.py하나로도 플랫폼을 만들수 있습니다. 그러나 FastAPI도 실무 표준 구조가 존재하므로 이 구조만 기억하면 됩니다.
...
![[Pasted image 20260309174915.png]]
...
1️⃣ router : 데이터 처리 로직 (DB 접근, 비즈니스 로직)
```python
@router.post("/users/")
...
2️⃣ schema : 입력/출력 데이터 구조 정의 및 유효성 검사
```python
...
4️⃣ crud : 데이터 처리 로직 (DB 접근, 비즈니스 로직)
```python
...
전체 흐름도 시각화 
https://www.figma.com/design/T6Hf8Uww5OqyGYBU144pak/Untitled?t=3RgtA31k2IMpLWNc-0
...
| 사용자 목록 | GET /users/             | READ   |
| 사용자 조회 | GET /users/{id}         | READ   |
| 아이템 생성 | POST /users/{id}/items/ | CREATE |
| 아이템 목록 | GET /items/             | READ   |

`sql_app/main.py` == (`views.py`, `urls.py`)
```python
...
```
---
`sql_app/schemas.py` == (`forms.py`, `serializers.py`)
```python
...
---
`sql_app/database.py`  == (`settings.py`, `apps.py`)
```python
...
/users/    create user
![[Pasted image 20260309160904.png]]

/users/   read users
![[Pasted image 20260309161227.png]]

/users/pk   read users
![[Pasted image 20260309161251.png]]

/users/pk/items/  create item for user
![[Pasted image 20260309161556.png]]

/items/ read items
![[Pasted image 20260309162554.png]]
