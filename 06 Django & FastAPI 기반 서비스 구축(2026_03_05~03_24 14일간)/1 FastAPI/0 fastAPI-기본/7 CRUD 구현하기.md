FastAPI는 적은 코드로 API를 빠르게 만들 수 있는 현대적인 Python 웹 프레임워크입니다.  
특히 요청 데이터 검증, 라우팅, 문서화(Swagger), 비동기 처리 같은 기능을 간단하게 구현할 수 있어서 학습용 예제에서는 하나의 `.py` 파일에 작성하는 경우가 많습니다.

하지만 실무에서는 보통 하나의 파일에 모든 코드를 넣지 않습니다.  
기능이 늘어나면 유지보수가 어려워지고, 협업과 테스트도 불편해지기 때문입니다.  
그래서 실제 프로젝트에서는 역할별로 파일과 폴더를 나누어 구성합니다.
...
├── models/              ← Pydantic 모델 또는 DB 관련 모델
│   ├── __init__.py
│   └── item.py
├── routes/              ← API 엔드포인트 정의
│   ├── __init__.py
...
각 파일의 역할
- `main.py`  
    FastAPI 앱을 만들고, 각 라우터를 등록하는 시작 파일입니다.
    
- `models/item.py`  
    요청/응답 데이터 형식을 정의하는 Pydantic 모델을 작성합니다.
    
- `routes/item.py`  
    `POST`, `GET`, `PUT`, `DELETE` 같은 API 엔드포인트를 작성합니다.
    
즉, 학습 단계에서는 한 파일로 시작할 수 있지만,  
프로젝트가 커질수록 파일을 역할별로 분리하는 방식이 더 적절합니다.
...
```
---
FastAPI List (Read All) `app/routes/item.py`
```python
# -----------------------
# LIST
# -----------------------
...
```
---
FastAPI Read One (Detail) `app/routes/item.py`
```python
# -----------------------
# DETAIL
# -----------------------
