### Data Class란?
	Python의 @dataclass는 데이터를 저장하기 위한 클래스를 간단하게 만들 수 있도록 도와주는 
	기능입니다.

일반적으로 클래스를 만들 때는 객체를 생성하기 위해  
`__init__`(초기화 함수)나 `__repr__`(객체 출력), `__eq__`(객체 비교) 같은 메서드를 직접 작성해야 합니다.

하지만 클래스 위에 `@dataclass`를 붙이면,  
Python이 이러한 메서드들을 자동으로 만들어 주기 때문에 코드를 훨씬 간단하게 작성할 수 있습니다.

즉, `@dataclass`는 데이터를 저장하는 목적의 클래스를 더 쉽고 깔끔하게 작성할 수 있도록 도와주는 도구라고 이해하시면 됩니다.


1️⃣ `__init__`: 객체가 생성될 때 실행되는 메서드 (정확한 표현: 던더 이닛)
```python
class Item:
...
2️⃣ `__repr__`: 객체를 출력할 때 보여줄 문자열을 정의 (정확한 표현: 던더 래퍼)
```python
class Item:
    def __init__(self, name, price):
        self.name = name
...
---
3️⃣ `__eq__`: 객체끼리 같은지 비교할 때 사용 (정확한 표현: 던더 이퀄)
```python
class Item:
    def __init__(self, name, price):
        self.name = name
...
### FastAPI에서 요청 데이터를 Data Class로 받기
	FastAPI에서 @dataclass를 사용하면 POST 요청으로 들어온 JSON 데이터를 객체로 
	변환해줍니다.
...
@dataclass
class Item: # 데이터를 저장하기 위한 Python 데이터 구조 클래스
    # 이름: 문자열 (필수)
    name: str
    # 가격: 실수형 숫자 (필수)
    price: float
...
# 필요한 모듈들을 불러옵니다.
from dataclasses import dataclass, field          
# 데이터 클래스를 만들기 위한 데코레이터와 필드 도구

from typing import List, Union                    
# 타입 힌트를 위해 리스트, 유니언 타입 사용

from fastapi import FastAPI                       
# FastAPI 애플리케이션 생성에 필요한 모듈
