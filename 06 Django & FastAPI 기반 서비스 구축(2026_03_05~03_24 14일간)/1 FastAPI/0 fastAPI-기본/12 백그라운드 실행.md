### 백그라운드 실행이란?
`사용자의 요청에 대한 응답을 먼저 빠르게 보내고, 시간이 오래 걸리는 작업은 서버 뒤에서 따로 실행하는 방식입니다. 즉, 사용자는 결과를 바로 확인할 수 있고, 서버는 추가 작업을 응답 이후에 조용히 처리합니다.
...
즉, 응답은 먼저 보내고, 시간이 걸리는 작업은 뒤에서 처리합니다
...
---
즉 쉽게 다시 정리하면 백그라운드 실행이란, 
사용자가 요청을 보냈을 때 화면에는 빠르게 응답을 보여주고, 시간이 오래 걸리는 작업
(예: 로그 기록, 이메일 전송 등)은 응답 이후에 조용히 처리함으로써 
사용자에게 지연을 느끼지 않게 하는 기술입니다.
...
```python
# background/background_task_example.py
...
def write_notification(email: str, message: str = ""):
    """
    백그라운드에서 실행될 작업 함수
    이메일과 메시지를 log.txt 파일에 기록합니다.
    """

    # 일부러 시간을 지연시켜 백그라운드 실행을 확인
    time.sleep(5)
...
서버 실행
```bash
uvicorn background.background_task_example:app --reload
```
...
결과가 바로 화면에서 출력됩니다
![[Pasted image 20260308212134.png]]

백그라운드 실행 확인 : 그리고 이후에 처리는 백그라운드에서 했습니다. log.txt
![[Pasted image 20260308212202.png]]
