...
2. **`axis=0`**
	...
	- 두 데이터프레임의 열 이름이 같아야 자연스럽게 이어집니다.  
참고: `axis=1`
- `"열을 따라"` 합친다는 뜻입니다. 즉, 옆으로 이어붙이는 것입니다.
- 같은 인덱스를 가진 `df_1`과 `df_2`의 열(column) 들이 나란히 합쳐집니다.
- 이 경우에는 인덱스가 기준이 되어 맞지 않는 인덱스는 `NaN`으로 채워집니다.
...
merged_df = pd.merge(df_1, df_2, on='customer_id', how='inner', suffixes=('_df1', '_df2'))

# 병합된 결과 CSV로 저장
merged_df.to_csv("merged_customers10.csv", index=False)
