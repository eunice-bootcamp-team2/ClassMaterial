...
---
#### 이 방식은 JavaScript 코드를 **기능별 역할로 나눈 구조(레이어 분리)** 입니다.
...
✔ 이벤트 / 화면 / 로직이 뒤섞임  
✔ 수정할 때 버그 폭발
그래서 처음부터 이런 식으로 구획을 나누는 것이 중요합니다.

브라우저에서 페이지가 열리면 다음 순서로 동작합니다:
```
DOMContentLoaded 발생  
↓  
init() 실행  
...
```JS
// 1) 설정(Config) 환경 준비 단계 ----------------------------
const axiosInstance = …;

// 2) 초기화 & 이벤트 바인딩(Init) 프로그램 시작점 ------------
function init() { … }
function bindUIEvents() { … }

// 3) 데이터 처리(Data)  서버 담당 구역 -------------------------
